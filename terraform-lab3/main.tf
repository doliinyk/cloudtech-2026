...
resource "aws_security_group" "web_sg" {
  ...
  ingress {
    ...
    # Доступ з усіх IP-адрес з метою навчання, сервер не планується запускати поза тестування
    cidr_blocks = ["0.0.0.0/0"]

    # Варіант обмеження доступу виключно вашою IP-адресою для запобігання брутфорс атакам
...
resource "aws_instance" "web_server" {
  ...
  instance_type = "t3.micro" # Тип інстансу в рамках Free Tier якого в мене нема
  subnet_id     = aws_subnet.subnet_a.id
